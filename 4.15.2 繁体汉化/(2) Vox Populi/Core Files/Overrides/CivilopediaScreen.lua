...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		local sText = "這個單位的[COLOR_CYAN]能力[ENDCOLOR]："; -- change to TXT_KEY_ later
		local function AnalyzeUnit(...)
			sText = sText .. AnalyzeObjectField(thisUnit, ...);
		end
		AnalyzeUnit("Domain", "Domains", "領域");
		--AnalyzeUnit("MinAreaSize");
		if thisUnit.MinAreaSize > 0 and thisUnit.MinAreaSize < 10 then sText = sText.."[NEWLINE][ICON_BULLET]在[COLOR_CYAN]海岸[ENDCOLOR]和[COLOR_CYAN]湖[ENDCOLOR]移動"; end
		if thisUnit.MinAreaSize >= 10 then sText = sText.."[NEWLINE][ICON_BULLET]只能在[COLOR_CYAN]海岸[ENDCOLOR]移動"; end
		AnalyzeUnit("CombatClass", "UnitCombatInfos", "類別");
		--AnalyzeUnit("IsMounted");
		if thisUnit.IsMounted == 1 then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_CYAN]騎乘[ENDCOLOR]單位"; end
		AnalyzeUnit("Special", "SpecialUnits", "特殊單位");
		AnalyzeUnit("GoodyHutUpgradeUnitClass", "UnitClasses", "從遺蹟升級到");
		AnalyzeUnit("PolicyType", "Policies", "政策要求");
		AnalyzeUnit("ProjectPrereq", "Projects", "計劃要求");
		AnalyzeUnit("SpaceshipProject", "Projects", "已由太空船計劃製作");
		AnalyzeUnit("ResourceType", "Resources", "要求");
		AnalyzeUnit("BeliefRequired", "Beliefs", "需要的信條");
		AnalyzeUnit("Trade");
		...
		AnalyzeUnit("Capture", "UnitClasses", "俘獲為");
		...
		AnalyzeUnit("DomainCargo", "Domains", "運載領域");
		AnalyzeUnit("SpecialCargo", "SpecialUnits", "特殊運載");
		AnalyzeUnit("SpecialUnitCargoLoad", "SpecialUnits", "特殊運載複核");
		AnalyzeUnit("Conscription", "");
		AnalyzeUnit("ExtraMaintenanceCost", "[ICON_GOLD]");
		--AnalyzeUnit("NoMaintenance");
		if thisUnit.NoMaintenance then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]無維護費[ENDCOLOR]"; end
		...
		AnalyzeUnit("LeaderPromotion", "UnitPromotions", "領袖晉升");
		...
		AnalyzeUnit("PromotionClass", "UnitCombatInfos", "晉升類別");
		...
		if thisUnit.ObsoleteTech ~= nil then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]"..Locale.Lookup(GameInfo.Technologies[thisUnit.ObsoleteTech].Description).." 后過時[ENDCOLOR]"; end
		--------------------
		local bShow = false;
		local sql = [[
		...
		for row in DB.Query(sql, thisUnit.Type) do
			if not bShow then sText = sText.."[NEWLINE]購買這個單位的前提[COLOR_CYAN]建築[ENDCOLOR]："; bShow = true; end -- change to TXT_KEY_ later
...
CivilopediaCategory[CategoryPromotions].SelectArticle = function( promotionID, shouldAddToList )
	...
	if promotionID ~= -1 then
		...
		-- Infixo more info
		local sText = "這個晉升的[COLOR_CYAN]能力[ENDCOLOR]："; -- change to TXT_KEY_ later
		...
		if thisPromotion.InstaHeal then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]立即治療[ENDCOLOR]"; end
		...
		if thisPromotion.AlwaysHeal then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]每回合都會治療[ENDCOLOR]"; end
		--AnalyzePromotion("HealOutsideFriendly");
		if thisPromotion.HealOutsideFriendly then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]友好領土外治療[ENDCOLOR]"; end
		...
		if thisPromotion.NoAttackInOcean == 1 then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]不能在海洋攻擊[ENDCOLOR]"; end
		...
		if thisPromotion.VisibilityChange ~= 0 then sText = sText.."[NEWLINE][ICON_BULLET]"..string.format("%+d", thisPromotion.VisibilityChange).." 可視范圍"; end
		...
		if thisPromotion.TechPrereq ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]"..Locale.Lookup(GameInfo.Technologies[thisPromotion.TechPrereq].Description).."[ENDCOLOR]"; end
		...
		if thisPromotion.BarbarianOnly == 1 then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_CYAN]僅限蠻族[ENDCOLOR]"; end
		AnalyzePromotion("CityStateOnly");
		--AnalyzePromotion("StrongerDamaged");
		if thisPromotion.StrongerDamaged == 1 then sText = sText.."[NEWLINE][ICON_BULLET]強化傷害"; end
		...
		if thisPromotion.GainsXPFromScouting == 1 then sText = sText.."[NEWLINE][ICON_BULLET]從探索中獲取 XP"; end
		--AnalyzePromotion("GainsXPFromPillaging");
		if thisPromotion.GainsXPFromPillaging == 1 then sText = sText.."[NEWLINE][ICON_BULLET]從掠奪中獲取 XP"; end
		--AnalyzePromotion("GainsXPFromSpotting");
		if thisPromotion.GainsXPFromSpotting == 1 then sText = sText.."[NEWLINE][ICON_BULLET]從探敵中獲取 XP"; end
		...
		if thisPromotion.IgnoreFeatureDamage == 1 then sText = sText.."[NEWLINE][ICON_BULLET]忽視特質傷害"; end
		AnalyzePromotion("ExtraTerrainDamage");
		--AnalyzePromotion("ExtraFeatureDamage");
		if thisPromotion.ExtraFeatureDamage == 1 then sText = sText.."[NEWLINE][ICON_BULLET]額外特質傷害"; end
		--AnalyzePromotion("CanCrossIce");
		if thisPromotion.CanCrossIce == 1 then sText = sText.."[NEWLINE][ICON_BULLET]可以進入浮冰單元格"; end
		--AnalyzePromotion("CanCrossMountains");
		if thisPromotion.CanCrossMountains == 1 then sText = sText.."[NEWLINE][ICON_BULLET]可以進入山嶽單元格"; end
		--AnalyzePromotion("CanCrossOceans");
		if thisPromotion.CanCrossOceans == 1 then sText = sText.."[NEWLINE][ICON_BULLET]可以進入海洋單元格"; end
		--AnalyzePromotion("EmbarkedDeepWater");
		if thisPromotion.EmbarkedDeepWater == 1 then sText = sText.."[NEWLINE][ICON_BULLET]可以海運進入海洋單元格"; end
		...
		for row in DB.Query("SELECT Domains.Description, UnitPromotions_Domains.Modifier, UnitPromotions_Domains.Attack, UnitPromotions_Domains.Defense FROM UnitPromotions_Domains INNER JOIN Domains ON UnitPromotions_Domains.DomainType = Domains.Type WHERE PromotionType = ?", thisPromotion.Type) do
			if row.Modifier and row.Modifier ~= 0 then sText = sText.."[NEWLINE][ICON_BULLET]"..string.format("%+d", row.Modifier).."% 強度，對[COLOR_CYAN]"..Locale.Lookup(row.Description).."[ENDCOLOR]"; end
			if row.Attack ~= 0 then sText = sText.."[NEWLINE][ICON_BULLET]"..string.format("%+d", row.Attack).."% 強度，在攻擊[COLOR_CYAN]"..Locale.Lookup(row.Description).."時[ENDCOLOR]"; end
			if row.Defense ~= 0 then sText = sText.."[NEWLINE][ICON_BULLET]"..string.format("%+d", row.Defense).."% 強度，在防禦[COLOR_CYAN]"..Locale.Lookup(row.Description).."時[ENDCOLOR]"; end
		end
		-- Classess
		for row in DB.Query("SELECT UnitClasses.Description, Modifier, Attack, Defense FROM UnitPromotions_UnitClasses INNER JOIN UnitClasses ON UnitPromotions_UnitClasses.UnitClassType = UnitClasses.Type WHERE PromotionType = ?", thisPromotion.Type) do
			...
			if row.Modifier ~= nil then sText = sText..string.format("%+d", row.Modifier).."% 強度"; end
			if row.Attack ~= nil then sText = sText..string.format(" %+d", row.Attack).."% 攻擊"; end
			if row.Defense ~= nil then sText = sText..string.format(" %+d", row.Defense).."% 防禦"; end
			sText = sText.."，對[COLOR_CYAN]"..Locale.Lookup(row.Description).."[ENDCOLOR]";
		end
		-- Combat Mods
		for row in DB.Query("SELECT UnitCombatInfos.Description, Modifier FROM UnitPromotions_UnitCombatMods INNER JOIN UnitCombatInfos ON UnitPromotions_UnitCombatMods.UnitCombatType = UnitCombatInfos.Type WHERE PromotionType = ?", thisPromotion.Type) do
			sText = sText.."[NEWLINE][ICON_BULLET]"..string.format("%+d", row.Modifier).."% 強度，對[COLOR_CYAN]"..Locale.Lookup(row.Description).."[ENDCOLOR]";
		end
		-- Terrains
		for row in DB.Query("SELECT TerrainType, Attack, Defense, DoubleMove, Impassable, PassableTech FROM UnitPromotions_Terrains WHERE PromotionType = ?", thisPromotion.Type) do
			...
			if row.DoubleMove then sText = sText.."[NEWLINE][ICON_BULLET]雙倍移動力，在 "..sTerrain; end
			if row.Impassable then
				sText = sText.."[NEWLINE][ICON_BULLET]不能進入[COLOR_NEGATIVE_TEXT]"..sTerrain.."[ENDCOLOR]";
				if row.PassableTech ~= nil then sText = sText.."，直到[COLOR_CYAN]"..Locale.Lookup(GameInfo.Technologies[row.PassableTech].Description).."[ENDCOLOR]"; end
			end
			if row.Attack ~= 0 or row.Defense ~= 0 then
				sText = sText.."[NEWLINE][ICON_BULLET]";
				if row.Attack ~= 0 then sText = sText..string.format("%+d", row.Attack).."% 攻擊"; end
				if row.Defense ~= 0 then sText = sText..string.format(" %+d", row.Defense).."% 防禦"; end
				sText = sText.."，在 "..sTerrain;
			end
		end
		-- Features
		for row in DB.Query("SELECT FeatureType, Attack, Defense, DoubleMove, Impassable, PassableTech FROM UnitPromotions_Features WHERE PromotionType = ?", thisPromotion.Type) do
			...
			if row.DoubleMove then sText = sText.."[NEWLINE][ICON_BULLET]雙倍移動力，在 "..sFeature; end
			if row.Impassable then
				sText = sText.."[NEWLINE][ICON_BULLET]不能進入[COLOR_NEGATIVE_TEXT]"..sFeature.."[ENDCOLOR]";
				if row.PassableTech ~= nil then sText = sText.."，直到[COLOR_CYAN]"..Locale.Lookup(GameInfo.Technologies[row.PassableTech].Description).."[ENDCOLOR]"; end
			end
			if row.Attack ~= 0 or row.Defense ~= 0 then
				sText = sText.."[NEWLINE][ICON_BULLET]";
		...
		for yieldType, values in pairs(pillageYields) do
			...
			if flatValue ~= 0 or scalingValue ~= 0 then
				...
				if flatValue ~= 0 then
					sText = sText.."[NEWLINE][ICON_BULLET]掠奪獎勵 "..string.format("%+d", flatValue).." "..sYieldIcon.." "..sYieldDesc;
				end
				if scalingValue ~= 0 then
					sText = sText.."[NEWLINE][ICON_BULLET]掠奪獎勵 "..string.format("%+d", scalingValue).." "..sYieldIcon.." "..sYieldDesc.."，隨時代遞增";
				end
			end
		end
		-- Negatives at the end
		if thisPromotion.CannotBeChosen then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]無法被選擇[ENDCOLOR]"; end
		if thisPromotion.LostWithUpgrade then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]升級時丟失[ENDCOLOR]"; end
		-- Promo lines (Unit Combats)
		sText = sText.."[NEWLINE][COLOR_CYAN]戰鬥類型[ENDCOLOR]符合這個晉升："; -- change to TXT_KEY_ later
		...
		-- Units list
		sText = sText.."[NEWLINE]給予這個晉升的[COLOR_CYAN]單位[ENDCOLOR]："; -- change to TXT_KEY_ later
...
-- Vox Populi end
		...
		-- Infixo more info
		local sText = "這個建築的[COLOR_CYAN]特質[ENDCOLOR]："; -- change to TXT_KEY_ later
		-- Generic info from main table
		...
		if thisBuilding.CapitalOnly == 1 then sText = sText.."[NEWLINE][ICON_BULLET]只允許[ICON_CAPITAL][COLOR_CYAN]首都[ENDCOLOR]"; end
		--AnalyzeBuilding("UnlockedByBelief");
		if thisBuilding.UnlockedByBelief then sText = sText.."[NEWLINE][ICON_BULLET]由[COLOR_CYAN]信條[ENDCOLOR]解鎖"; end
		--AnalyzeBuilding("UnlockedByLeague");
		if thisBuilding.UnlockedByLeague then sText = sText.."[NEWLINE][ICON_BULLET]由[COLOR_CYAN]世界議會[ENDCOLOR]解鎖"; end
		--AnalyzeBuilding("PolicyBranchType");
		if thisBuilding.PolicyBranchType ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]"..Locale.Lookup(GameInfo.PolicyBranchTypes[thisBuilding.PolicyBranchType].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("PolicyType");
		if thisBuilding.PolicyType ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]"..Locale.Lookup(GameInfo.Policies[thisBuilding.PolicyType].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("IsCorporation");
		if thisBuilding.IsCorporation == 1 then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_CYAN]公司[ENDCOLOR]"; end
		AnalyzeBuilding("TeamShare");
		AnalyzeBuilding("Water");
		--AnalyzeBuilding("MinAreaSize");
		if thisBuilding.MinAreaSize > 0 and thisBuilding.MinAreaSize < 10 then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]湖[ENDCOLOR]"; end
		if thisBuilding.MinAreaSize >= 10 then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]海岸[ENDCOLOR]"; end
		...
		if thisBuilding.NearbyTerrainRequired ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]需要 [COLOR_CYAN]"..Locale.Lookup(GameInfo.Terrains[thisBuilding.NearbyTerrainRequired].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("ProhibitedCityTerrain");
		if thisBuilding.ProhibitedCityTerrain ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]被禁止的 [COLOR_NEGATIVE_TEXT]"..Locale.Lookup(GameInfo.Terrains[thisBuilding.ProhibitedCityTerrain].Description).."[ENDCOLOR]"; end
		...
		if thisBuilding.ObsoleteTech ~= nil then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]"..Locale.Lookup(GameInfo.Technologies[thisBuilding.ObsoleteTech].Description).." 後過時[ENDCOLOR]"; end
		--AnalyzeBuilding("EnhancedYieldTech");
		if thisBuilding.EnhancedYieldTech ~= nil then sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.Technologies[thisBuilding.EnhancedYieldTech].Description).."的額外收益[ENDCOLOR]"; end
		AnalyzeBuilding("科技提升旅游業績", "[ICON_TOURISM]");
		--AnalyzeBuilding("FreeBuilding");
		if thisBuilding.FreeBuilding ~= nil then sText = sText.."[NEWLINE]所有城市[ICON_BULLET]無償的[COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.BuildingClasses[thisBuilding.FreeBuilding].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("FreeBuildingThisCity");
		if thisBuilding.FreeBuildingThisCity ~= nil then sText = sText.."[NEWLINE]所有城市[ICON_BULLET]無償的[COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.BuildingClasses[thisBuilding.FreeBuildingThisCity].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("FreePromotion");
		if thisBuilding.FreePromotion ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]所有單位收到[COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.UnitPromotions[thisBuilding.FreePromotion].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("TrainedFreePromotion");
		if thisBuilding.TrainedFreePromotion ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]這個城市訓練的單位收到[COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.UnitPromotions[thisBuilding.TrainedFreePromotion].Description).."[ENDCOLOR]"; end
		--AnalyzeBuilding("FreePromotionRemoved");
		if thisBuilding.FreePromotionRemoved ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]移除[COLOR_NEGATIVE_TEXT]"..Locale.Lookup(GameInfo.UnitPromotions[thisBuilding.FreePromotionRemoved].Description).."[ENDCOLOR]"; end
		AnalyzeBuilding("ReplacementBuildingClass");
		--AnalyzeBuilding("FreeGreatWork");
		if thisBuilding.FreeGreatWork ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]無償的[COLOR_POSITIVE_TEXT]"..Locale.Lookup(GameInfo.GreatWorks[thisBuilding.FreeGreatWork].Description).."[ENDCOLOR]"; end
		...
		if thisBuilding.NeedBuildingThisCity ~= nil then sText = sText.."[NEWLINE][ICON_BULLET]城市需要[COLOR_NEGATIVE_TEXT]"..Locale.Lookup(GameInfo.BuildingClasses[thisBuilding.NeedBuildingThisCity].Description).."[ENDCOLOR]"; end
		...
		if thisBuilding.EventChoiceRequiredActive ~= nil then
			...
			sText = sText.."[NEWLINE][ICON_BULLET]需要事件選擇: [COLOR_POSITIVE_TEXT]"..Locale.Lookup(sDesc).."[ENDCOLOR]"
		end
		...
		if thisBuilding.CityEventChoiceRequiredActive ~= nil then
			...
			sText = sText.."[NEWLINE][ICON_BULLET]需要城市事件選擇: [COLOR_POSITIVE_TEXT]"..Locale.Lookup(sDesc).."[ENDCOLOR]"
		end
		...
		if thisBuilding.FreeStartEra ~= nil then
			...
			sText = sText.."[NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]"..Locale.Lookup(sDesc).." 後免費[ENDCOLOR]"
		end
		...
		if thisBuilding.MaxStartEra ~= nil then
			...
			sText = sText.."[NEWLINE][ICON_BULLET]僅到[COLOR_NEGATIVE_TEXT]"..Locale.Lookup(sDesc).."[ENDCOLOR]"
		end
		-------------------
			...
			if sRefTable == "" then
				...
				for row in DB.Query(sql, thisBuilding.Type) do
					...
					elseif row.Description == sGroup then
						...
						sLocText = sLocText.."來自"..Locale.Lookup(sGroup);
						sLocText = sLocText..string.format("[NEWLINE][ICON_BULLET]%+d%s ", row.Yield, row.IconString);
						sGroup = row.Description;
					end
				end
				if sGroup ~= "" then sLocText = sLocText.."來自"..Locale.Lookup(sGroup); end
			end
			if sLocText ~= "" then
		...
		AnalyzeBuildingYields("Building_TerrainYieldChanges",   	     "TerrainType", 		"Terrains",			"", 		"獎勵來自[COLOR_CYAN]地形[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_FeatureYieldChanges",            "FeatureType",       	"Features",			"", 		"獎勵來自[COLOR_CYAN]特質[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_ResourceYieldChanges",           "ResourceType",      	"Resources", 		"", 		"獎勵來自[COLOR_CYAN]資源[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_SeaResourceYieldChanges", 		 "", 					"", 				"", 		"獎勵來自[COLOR_CYAN]海洋資源[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_ImprovementYieldChanges",        "ImprovementType",   	"Improvements",  	"", 		"獎勵來自[COLOR_CYAN]改良設施[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_PlotYieldChanges",               "PlotType",          	"Plots",         	"", 		"獎勵來自[COLOR_CYAN]單元格[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_LakePlotYieldChanges",           "",                  	"",             	"", 		"獎勵來自[COLOR_CYAN]湖[ENDCOLOR]單元格：");
		AnalyzeBuildingYields("Building_RiverPlotYieldChanges", 		 "", 					"", 				"", 		"獎勵來自[COLOR_CYAN]河流[ENDCOLOR]單元格：");
		AnalyzeBuildingYields("Building_SeaPlotYieldChanges", 			 "", 					"", 				"", 		"獎勵來自[COLOR_CYAN]海洋[ENDCOLOR]單元格：");
		AnalyzeBuildingYields("Building_BuildingClassLocalYieldChanges", "BuildingClassType", 	"BuildingClasses", 	"Change", 	"獎勵來自[COLOR_CYAN]建築[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_SpecialistYieldChangesLocal",    "SpecialistType", 		"Specialists", 		"", 		"獎勵來自[COLOR_CYAN]專家[ENDCOLOR]：");
		-- global changes
		AnalyzeBuildingYields("Building_ImprovementYieldChangesGlobal",  "ImprovementType",   	"Improvements",    	"", 		"獎勵來自[COLOR_POSITIVE_TEXT]所有城市[ENDCOLOR]的[COLOR_CYAN]改良設施[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_BuildingClassYieldChanges",      "BuildingClassType", 	"BuildingClasses", 	"Change", 	"獎勵來自[COLOR_POSITIVE_TEXT]所有城市[ENDCOLOR]的[COLOR_CYAN]建築[ENDCOLOR]：");
		AnalyzeBuildingYields("Building_SpecialistYieldChanges", 		 "SpecialistType", 		"Specialists", 		"", 		"獎勵來自[COLOR_POSITIVE_TEXT]所有城市[ENDCOLOR]的[COLOR_CYAN]專家[ENDCOLOR]：");
		-------------------
		UpdateTextBlock( sText, Controls.ExtendedLabel,  Controls.ExtendedInnerFrame,  Controls.ExtendedFrame );
		-- end Infixo
...
CivilopediaCategory[CategoryPeople].SelectArticle =  function( rawPeopleID, shouldAddToList )
	...
	if rawPeopleID < 1000 then
		if rawPeopleID ~= -1 then
			...
			-- Infixo Extended info
			local sText = "這個專家可去的[COLOR_CYAN]建築[ENDCOLOR]："; -- change to TXT_KEY_ later
			...
			sText = sText.."[NEWLINE]以下建築給予的[COLOR_CYAN]全局收益[ENDCOLOR]：";
			local sql = [[
			...
			sText = sText.."[NEWLINE]以下建築給予的[COLOR_CYAN]本地收益[ENDCOLOR]：";
			local sql = [[
			...
			sText = sText.."[NEWLINE]以下[COLOR_CYAN]科技[ENDCOLOR]可增加的收益：";
			local sql = [[
		...
		if greatPersonID ~= -1 then
			...
			if sSpecialistType ~= "" then
				...
				local sText = "這個人物可加速以下[COLOR_CYAN]建築[ENDCOLOR][ICON_GREAT_PEOPLE]："; -- change to TXT_KEY_ later
				local bShow = false;
				for row in GameInfo.Buildings() do
...
CivilopediaCategory[CategoryResources].SelectArticle = function( resourceID, shouldAddToList )
	...
	if resourceID ~= -1 then
		...
		if thisResource then
			...
			--	end
			--end
			sText = "這個資源提升以下[COLOR_CYAN]建築[ENDCOLOR]：";
			local sql = [[
