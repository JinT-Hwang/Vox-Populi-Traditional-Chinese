...
UPDATE Language_ZH_HANT_HK
SET Text = '當您完成一項科技時，每位[ICON_RELIGION]宗教信徒，+2 [ICON_GOLDEN_AGE]黃金時期點數、[ICON_PEACE]信仰和[ICON_CULTURE]文化 (最多 250 位信徒)。[NEWLINE]解鎖[COLOR_POSITIVE_TEXT]教皇檔案[ENDCOLOR]國家奇跡([ICON_RELIGION]聖地 +5 [ICON_RESEARCH]科技；解鎖[COLOR_POSITIVE_TEXT]改革信條[ENDCOLOR])。'
