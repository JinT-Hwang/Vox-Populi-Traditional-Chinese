...
-- Start a黃金時代
UPDATE Language_ZH_HANT_HK
SET Text = '+{1_Num} [ICON_GOLDEN_AGE]黃金時代點數[NEWLINE][NEWLINE]這個指令會消耗大藝術家和創造黃金時代點數，可能會觸發[ICON_GOLDEN_AGE]黃金時代 (額外的[ICON_PRODUCTION]產能，[ICON_GOLD]金幣和[ICON_CULTURE]文化)。 以過去 10 個回合的[ICON_TOURISM]旅遊業績和[ICON_GOLDEN_AGE]黃金時代點數產出為基礎進行調整，並且每擁有一組[COLOR_POSITIVE_TEXT]已啟動主題[ENDCOLOR]的巨作，就會增加 20%。'
