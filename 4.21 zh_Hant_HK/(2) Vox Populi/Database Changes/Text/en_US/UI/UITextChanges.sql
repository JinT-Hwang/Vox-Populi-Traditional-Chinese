...
-- Happiness
UPDATE Language_ZH_HANT_HK
SET Text = '您的支持率至少有 50%。[ICON_HAPPINESS_1]幸福的城市會[ICON_FOOD]成長的更快，但是[ICON_HAPPINESS_3]不滿的城市的[ICON_FOOD]成長，軍事單位成本和開拓者成本會遭受懲罰。'
...
UPDATE Language_ZH_HANT_HK
SET Text = '[NEWLINE][ICON_BULLET][ICON_HAPPINESS_1]「敬後之日」修正：{1_Num}%'
...
UPDATE Language_ZH_HANT_HK
SET Text = '允許其他玩家的軍事或平民單位通過自己的領土(持續 {1_Num} 回合)。[NEWLINE][NEWLINE]允許其他文明通過自己的領土會增加對該文明的[ICON_TOURISM]旅遊業績修正值。[NEWLINE][NEWLINE]注意：屬於不同玩家的軍事單位永遠不能疊加。'
...
UPDATE Language_ZH_HANT_HK
SET Text = '在遊戲中，您將創造「工人」--非軍事單位，他們將「改良」您城市周圍的土地，提高土地的生產力或提供附近「資源」的使用權。改良項目包括農場、交易站、鋸木廠、採石場、礦場等等。在戰爭期間，您的敵人可能會「掠奪」(破壞)您的改良設施。被掠奪的改良設施在工人「修繕」之前是無效的。[NEWLINE][NEWLINE]{TXT_KEY_BUILDINGS_SPECIALISTS_HEADING2_BODY}'
