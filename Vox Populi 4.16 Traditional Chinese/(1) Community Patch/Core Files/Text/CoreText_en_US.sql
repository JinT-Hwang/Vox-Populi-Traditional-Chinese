...
VALUES		('TXT_KEY_PRODMOD_WONDER_UNITPROMOTION',						'[NEWLINE][ICON_BULLET]單位駐軍的奇跡修正：{1_Num}%'),
			...
			('TXT_KEY_RELIGION_TOOLTIP_EXTENDED',							'{1_ReligionIcon} {2_NumFollowers} 信徒({3_TotalPressure} + {4_PressurePerTurn} 壓力，來自 {5_Num} 源頭城市)'),
...
VALUES	('TXT_KEY_GAME_OPTION_BARB_GG_GA_POINTS',		'蠻族點數'),
		('TXT_KEY_GAME_OPTION_BARB_GG_GA_POINTS_HELP',	'允許所有玩家從與蠻族的戰鬥中累積大將軍和海軍上將點數。');
...
UPDATE Language_ZH_Hant_HK
SET Text = '當與蠻族部隊交戰時，您訓練較差的部隊會獲得經驗值。然而，任何已經獲得 [COLOR_YELLOW]45 經驗[ENDCOLOR]的單位(或已經兌換了這麼多的晉升點數)不再會從與蠻族的戰鬥中獲得經驗。'
...
UPDATE Language_ZH_Hant_HK
SET Text = '你就是這樣的盟友？世界上其他國家的領袖都會聽到你們的背叛。[NEWLINE][NEWLINE]([COLOR_WARNING_TEXT]他們公開譴責我們！[ENDCOLOR])'
