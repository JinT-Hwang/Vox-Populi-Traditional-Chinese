...
UPDATE Language_ZH_Hant_HK
SET Text = '每個偉人類型可消耗用於在您的文明邊界內的單元格上建造一個特殊改良設施。特殊改良設施的效果取決於創造它的偉人 - 例如，大工程師的特殊改良設施會產出產能，而大商業家的特殊改良設施則會產出現金。[COLOR_YELLOW]隨著遊戲的進行，特殊改良的產量會隨著科技、政策、建築和信條而增加。[ENDCOLOR][NEWLINE][NEWLINE]特殊改良設施必須經過公民工作才能產生任何效果。此外，特殊改良設施可以像其他改良設施一樣被掠奪和修繕。如果建造在資源之上，只有當該資源是戰略資源時，特殊改良設施才會提供該資源。[NEWLINE][NEWLINE]請注意，您必須將偉人走出城市，並走入您的領土，才能建造特殊改良設施。'
