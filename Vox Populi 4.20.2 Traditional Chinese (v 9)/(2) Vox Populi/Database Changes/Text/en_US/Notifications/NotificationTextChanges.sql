...
-- City-State quests
UPDATE Language_ZH_HANT_HK
SET Text = '{2_CivName:textkey} 與 {1_TargetName:textkey} 之間爆發戰爭'
WHERE Tag = 'TXT_KEY_NOTIFICATION_SUMMARY_QUEST_KILL_CITY_STATE';

UPDATE Language_ZH_HANT_HK
SET Text = '{3_TargetName:textkey} 因外交考量, 向 {1_CivName:textkey} 宣戰。通過 [COLOR_NEGATIVE_TEXT]摧毀[ENDCOLOR] {3_CivName:textkey} 以從 {1_TargetName:textkey} 獲得 [COLOR_POSITIVE_TEXT]全部[ENDCOLOR] 任務獎勵，或者通過讓兩個城邦 [COLOR_POSITIVE_TEXT]結盟[ENDCOLOR] 獲得 [COLOR_POSITIVE_TEXT]部分[ENDCOLOR] 任務獎勵。國際壓力將在 [COLOR_POSITIVE_TEXT]{2_Num}[ENDCOLOR] 回合內迫使和平，所以如果您打算幹預，請迅速行動！'
...
UPDATE Language_ZH_HANT_HK
SET Text = '{2_CivName:textkey} 和 {1_TargetName:textkey} 之間的戰爭已經結束，{2_CivName:textkey} 成為明顯的勝利者。你的顧問擔心這個結果可能會導致未來的衝突。'
WHERE Tag = 'TXT_KEY_NOTIFICATION_QUEST_COMPLETE_KILL_CITY_STATE';

-- City Revolt Brewing
UPDATE Language_ZH_HANT_HK
SET Text = '由於您的帝國的支持率低於 35%，一座城市將在 [COLOR_NEGATIVE_TEXT]{1_Turns}[ENDCOLOR] 回合內起義。根據當前的不滿度和文化/意識形態壓力，最有可能起義的城市是 {2_CityName}，而它將加入 {3_CivName}。努力讓你的帝國的幸福度盡快脫離這個範圍！'
WHERE Tag = 'TXT_KEY_NOTIFICATION_POSSIBLE_CITY_REVOLT';

-- Revolutionary Unrest Brewing
UPDATE Language_ZH_HANT_HK
SET Text = '由於您的帝國的支持率低於 35%，一座城市將在 [COLOR_NEGATIVE_TEXT]{1_Turns}[ENDCOLOR] 回合內起義。根據當前的不滿度，最有可能起義的城市是 {2_CityName}。如果這座城市反抗，它將恢復 {3_CivName} 的主權。努力讓你的帝國的幸福度盡快脫離這個範圍！'
WHERE Tag = 'TXT_KEY_NOTIFICATION_POSSIBLE_CITY_REVOLUTION_CP';

UPDATE Language_ZH_HANT_HK
SET Text = '由於您的帝國的支持率低於 35%，一座城市將在 [COLOR_NEGATIVE_TEXT]{1_Turns}[ENDCOLOR] 回合內起義。根據當前的不滿度，最有可能起義的城市是 {2_CityName}。如果這座城市起義，它將成為一個獨立的城邦。。努力讓你的帝國的幸福度盡快脫離這個範圍！'
WHERE Tag = 'TXT_KEY_NOTIFICATION_POSSIBLE_CITY_REVOLUTION_CP_FREE_CITY';

-- A City Revolts!
UPDATE Language_ZH_HANT_HK
SET Text = '由於您的帝國的支持率低於 35%，城市 {1_CityName} 已起義並加入了 {2_CivName}！'
WHERE Tag = 'TXT_KEY_NOTIFICATION_CITY_REVOLT';

-- Rebels Appear!
UPDATE Language_ZH_HANT_HK
SET Text = '由於您的帝國的支持率低於 35%，你的領土上發生了起義！'
